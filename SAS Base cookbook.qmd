...
This chapter shows how to rename variables, add descriptive labels, change display formats, and control storage length.

### RENAME=

RENAME=: changes variable names when reading/writing a dataset.

```{sas}
data work.birth_new;
    set work.birthweight (rename = (lwt = mom_weight age = mom_age));
run;
```

rename = (<old = new>) → Renames variables as data are read/written. Here `lwt` becomes `mom_weight`; `age` becomes `mom_age`.
...
```{sas}
data work.birth_labels;
    set work.birthweight;
    label bwt = "Infant birth weight (grams)"
          smoke = "Mother smoked during pregnancy";
run;

proc print data = work.birth_labels (obs = 5) label;
run;
```

label <var> = <"text">; → Adds descriptions for variables. Appears in procedures like PROC PRINT (when label option used).

### FORMAT

FORMAT: controls how values are displayed (not stored).

```{sas}
data work.birth_format;
    set work.birthweight;
    format bwt comma8.;
    format age 3.;
run;

proc print data = work.birth_format (obs = 5);
run;
```

format bwt comma8.; → Shows, e.g., 3200 as 3,200.
format age 3.; → "numeric width 3; 0 decimals", i.e. SAS will display age in a field that’s 3 characters wide, right‑aligned, with no decimal places.

### LENGTH

LENGTH: defines storage length of character variables.

```{sas}
data work.birth_length;
    length race $15;     /* allocate 15 characters */
    set work.birthweight;
run;
```

length var $<n>; → sets storage size (n characters) for character variable.

Must appear **before** the variable is created, otherwise SAS defaults to first encountered length.
